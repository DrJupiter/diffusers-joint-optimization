...
#from symengine import Matrix, Symbol, Lambdify as lambdify 
    ...
    def __init__(self, variable, parameters, diffusion, diffusion_matrix=None, initial_value=0., module='jax', integral_form=False, integral_decomposition='cholesky', diffusion_dimension=SDEDimension.SCALAR, diffusion_matrix_dimension=SDEDimension.SCALAR):
        ...
        if integral_form:
            ...
            if integral_decomposition == 'cholesky':
                ...
                raise NotImplementedError(f'Solution not implemented for the decomposition form {integral_decomposition}')
...
if __name__ == "__main__":
    ...
    F = sympy.Matrix.diag([sympy.cos(t*drift_param[0]) + t * (drift_param[1]) + t**2 * drift_param[2]]*n)
    L = sympy.Matrix.diag([sympy.sin(t)*diffusion_param[0] + t * diffusion_param[1]]*n)
    Q = sympy.Matrix.eye(n)
