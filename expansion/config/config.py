...
class SDEConfig:
    ...
    drift_parameters = Matrix([sympy.symbols("f1 f2 f3 f4")])
    diffusion_parameters = Matrix([sympy.symbols("l1 l2 l3")])
    
    drift =-10*variable**2 * drift_parameters[0]**2 + sum(drift_parameters[1:])**2
    k = 1 * diffusion_parameters[0]**2 + sum(diffusion_parameters[1:])**2
